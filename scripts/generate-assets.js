...
function processSounds(p) {
    const files = getFiles(p).filter((elm) => {
        return elm.match(/.*\.(mp3|ogg|wav)/gi);
    });
    let sounds = '';
    const value = files.reduce((str, sound) => {
        const url = sound.replace(cwd, '../..');
        const soundName = path.basename(sound, path.extname(sound));
        str += `'${soundName}':require('${url}'),`;
        sounds += `'${camelize(soundName)}':'${soundName}',`;
        return str;
    }, '');
    return [value, sounds];
}
